...
%token <st> IDENTIFIER CONSTANT STRING_LITERAL SIZEOF CONSTANT_INT CONSTANT_DOUBLE
%token <st> PTR_OP INC_OP DEC_OP LEFT_OP RIGHT_OP LE_OP GE_OP EQ_OP NE_OP
%token <st> AND_OP OR_OP MUL_ASSIGN DIV_ASSIGN MOD_ASSIGN ADD_ASSIGN
%token <st> SUB_ASSIGN LEFT_ASSIGN RIGHT_ASSIGN AND_ASSIGN
%token <st> XOR_ASSIGN OR_ASSIGN TYPE_NAME

%token <st> CHAR INT DOUBLE VOID BOOL 

%token <st> CASE IF ELSE SWITCH WHILE DO FOR GOTO CONTINUE BREAK RETURN

%token <st> TRUE FALSE

%token <st> ';' ',' ':' '=' '[' ']' '.' '&' '!' '~' '-' '+' '*' '/' '%' '<' '>' '^' '|' '?' '{' '}' '(' ')'

%type <st> primary_expression postfix_expression argument_expression_list unary_expression unary_operator
%type <st> multiplicative_expression additive_expression shift_expression relational_expression equality_expression
%type <st> and_expression exclusive_or_expression inclusive_or_expression logical_and_expression logical_or_expression
%type <st> assignment_expression assignment_operator expression

%type <st> declaration init_declarator_list init_declarator type_specifier

%type <st> declarator 

%type <st> parameter_list parameter_declaration identifier_list
%type <st> abstract_declarator initializer initializer_list designation designator_list
%type <st> designator statement labeled_statement compound_statement block_item_list block_item expression_statement
%type <st> selection_statement iteration_statement jump_statement translation_unit external_declaration function_definition
%type <st> declaration_list
